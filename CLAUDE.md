...
- [ ] **Étape 2 — LOT C2** : prérequis technique (redresser un rectangle **tourné** — `warp_region` ne sait faire que l'aligné sur les axes), puis clic sur une zone → zoom, tracé des polylines (trait élastique, double-clic pour clore), undo/redo de profondeur 1 sur 3 actions, sélection/suppression d'un cordon, retour vue globale avec report des cordons sur **toutes** les zones
