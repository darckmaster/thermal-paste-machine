...
- [ ] **Étape 2 — LOT B** : modèle de données + sérialisation JSON (plateau, zones, cordons en mm relatifs à la zone, paramètres globaux vitesse déplacement/extrusion, autosave dans un fichier temporaire + enregistrement définitif)
- [ ] **Étape 2 — LOT C** : l'IHM (nouvel écran, contrôle de cohérence visuel, sélection de zone, tracé des polylines avec undo/redo de profondeur 1, report des cordons sur toutes les zones). Tests automatiques envisagés avec `pytest-qt`. Copies d'écran à produire pour le manuel utilisateur à ce moment-là
- [ ] **Seuils de zone à déplacer ?** `ZONE_DIAGONAL_TOLERANCE_MM` (5 mm) et `ZONE_MAX_ROTATION_DEG` (10°) sont dans `vision.py` — question ouverte : les remonter dans la fenêtre de paramètres du lot B, à côté des vitesses
...
| Fixture de test caméra | `CAMERA_INDEX` de `local_config.json`, jamais l'index 0 en dur | Sur PC de dev, l'index 0 est la webcam intégrée : les tests validaient un matériel qui n'est pas celui du projet (corrigé le 2026-08-01) |
